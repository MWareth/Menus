...
const SEED_OUT = path.join(here, "src", "seed.js");

const src = fs.readFileSync(SOURCE, "utf8");
const css = src.match(/<style id="app-style">([\s\S]*?)<\/style>/)[1].trim();
let js = src.match(/<script id="app-code">([\s\S]*?)\n<\/script>/)[1].trim();

/* ---- 0. the starting board comes from the same place the screens do ----
   The site writes this into the database on its very first request. Taking it
   from the app rather than keeping a second copy by hand means the hosted
   board opens on exactly the board the team last saw. */
const state = JSON.parse(
  src.match(/<script id="app-state" type="application\/json">\s*([\s\S]*?)\s*<\/script>/)[1]
);
/* the PIN everyone starts on, so the server can tell who has not changed theirs */
const DEFAULT_PIN_HASH =
  "d4165ae679b414636d9b7ff6fb6e1ee8b4824dfee69e3b2fc47fdfc259d38117";
state.defaultPinHash = DEFAULT_PIN_HASH;
fs.writeFileSync(SEED_OUT, `/**
 ...
 */
export const SEED = ${JSON.stringify(state, null, 2)};
`);
...
/* ---- 9. a file is an ordinary download here, not a permission prompt ---- */
const hoStart = js.indexOf("  function handOver(file) {");
const hoEnd = js.indexOf("\n  }\n", js.indexOf("dl.save(", hoStart)) + 5;
if (hoStart < 0) throw new Error("handOver() not found");
js = js.slice(0, hoStart) + `  function handOver(file) {
    try {
      var url = URL.createObjectURL(new Blob([file.bytes], { type: "application/pdf" }));
      var a = document.createElement("a");
      a.href = url; a.download = file.filename; a.rel = "noopener";
      document.body.appendChild(a); a.click(); a.remove();
      setTimeout(function () { URL.revokeObjectURL(url); }, 8000);
      toast(file.filename + " saved.");
    } catch (e) { toast("Could not save it here.", true); }
  }
` + js.slice(hoEnd);
...
console.log("built src/seed.js —", state.batches.length, "lots,", state.team.length, "people");
